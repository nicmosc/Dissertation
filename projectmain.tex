...
\end{flalign*}
essentially turning the sentence into a rhetorical question. The same happens when we try to describe the state of an object or person like
\begin{flalign*}
...
\begin{figure}[h]
	\centering
    \includegraphics[scale=0.9]{chapter3/mt-outline}
    \caption{Machine Translation module design}
    \label{fig:mt-outline}
\end{figure}

The design of the pipeline was heavily inspired by the approach used in \citex{paper:drt}, while the tree transformation 
...
\begin{figure}[h]
	\centering
    \includegraphics[scale=0.75]{chapter3/flowchart}
...
%	\begin{tikzpicture}
%	
%		\begin{axis}[xlabel={Length}, ylabel=Accuracy, legend pos=outer north east]
%		\addplot [only marks] table [y=Accuracy, x=Length] {\mydata};
%		\addplot [thick, red] table[
%    			y={create col/linear regression={y=Accuracy}}
%		]{\mydata}; % compute a linear regression from the input table
